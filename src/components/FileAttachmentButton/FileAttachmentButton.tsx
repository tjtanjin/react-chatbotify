...
const FileAttachmentButton = ({
	...
	const handleUpload = (event: ChangeEvent<HTMLInputElement>) => {
		...
		if (fileHandler != null) {
			...
			handleActionInput(currPath, fileNames.join(", "), false);
			fileHandler(files, {userInput: inputRef.current?.value as string, prevPath: getPrevPath(),
				injectMessage, openChat});
		}
	};
		...
		<>
			{blockAllowsAttachment ? (
				<label className="rcb-attach-button-enabled">
					...
					/>
					<span
						style={{backgroundImage: `url(${botOptions.footer?.fileAttachmentImage})`}}
				...
				<label className="rcb-attach-button-disabled">
					...
					<span 
						style={{
							backgroundImage: `url(${botOptions.footer?.fileAttachmentImage})`,
